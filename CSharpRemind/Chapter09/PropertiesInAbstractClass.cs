...
namespace CSharpRemind.Chapter09
{
    ...
    {
        static void MainPropertiesInAbstractClass()
        {
            Product product_1 = new MyProduct()
            { ProductDate = new DateTime(2018, 1, 10) };
            Console.WriteLine("Product: {0} Product date: {1}", product_1.SerialID,
                product_1.ProductDate);

            Product product_2 = new MyProduct()
            { ProductDate = new DateTime(2018, 2, 3) };

            Console.WriteLine("Product: {0} Product Date: {1}", product_2.SerialID,
